...
고령사회 진입에 따른 인지건강 관리 수요에 맞춰, 방문 요양보호사가 현장에서 스마트폰만으로 편리하게 활용할 수 있는 말하기형 인지훈련 및 보조 문진 웹서비스 [cite: 4, 9, 29, 67, 72]
...
**Token flow**는 재가돌봄 환경의 제약을 극복하고 고령층의 인지기능 저하를 지속적으로 점검하기 위한 음성 인터페이스 기반 인지케어 서비스입니다[cite: 12, 59, 68].  
어르신들이 화면 조작이나 필기 부담 없이 제시된 문항에 음성으로 답변하면 [cite: 82, 95][cite_start], 시스템이 Whisper STT 모델을 통해 답변을 텍스트로 변환한 뒤 [cite: 75, 84] [cite_start]정답 여부, 지연 반응 시간, 발화 지표(반복어, 문장 길이, 화행 적절성)를 자동 분석하여 체계적으로 기록합니다[cite: 75, 87].

### 주요 특징

- 🧩 **말하기형 인지훈련 퀴즈**: 날짜 말하기, 그림 설명, 상황 질문 답하기 등 음성 응답 중심 문항 제공 [cite: 81, 82]
- 📈 **STT 기반 음성 텍스트 변환**: Hugging Face 오픈소스 기반 Whisper 모델을 통한 고품질 한국어 변환 [cite: 84, 95, 138]
- 🧠 **발화 분석 지표 산출**: 반응 시간, 반복어 비율, 평균 문장 길이, 화행 적절성 스코어 분석 [cite: 87, 138]
- 👤 **개인화된 훈련 모드**: 검사 진행 이후 비교적 낮게 측정된 카테고리 영역을 선택 집중 훈련 
- 🔐 **사용자 및 수급자 관리**: 요양보호사 회원관리(세션 및 소셜) 및 담당 수급자 이력 연동 [cite: 138, 147]
- 📋 **시각화 리포트 및 PDF 발행**: 주간·월간 인지 변화 추이 그래프 및 자동 결과 보고서 다운로드 지원 [cite: 91, 92, 138]
